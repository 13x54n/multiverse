...
export class Resolver {
    ...
    public deploySrc(
        ...
        // Debug: Check which parameter is null
        const params = [immutables.build(), order.build(), r, vs, amount, trait, args]
        const paramNames = ['immutables', 'order', 'r', 'vs', 'amount', 'trait', 'args']
        
        for (let i = 0; i < params.length; i++) {
            if (params[i] === null || params[i] === undefined) {
                console.log(`[DEBUG] Parameter ${paramNames[i]} is null/undefined:`, params[i])
            }
        }

        return {
            to: this.srcAddress,
            data: this.iface.encodeFunctionData('deploySrc', params),
