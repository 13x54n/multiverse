...
import {ChainConfig, config} from './Eth2Monad.config'
...
describe('Resolving example', () => {
    ...
    // eslint-disable-next-line max-lines-per-function
    describe('Fill', () => {
        it('should swap Ethereum USDC -> Monad. Single fill only', async () => {
        ...
        it('should swap Ethereum USDC -> Monad. Multiple fills. Fill 100%', async () => {
            const initialBalances = await getBalances(
                config.chain.source.tokens.USDC.address,
                config.chain.destination.tokens.USDC.address
            )
        ...
        it('should swap Ethereum USDC -> Monad. Multiple fills. Fill 50%', async () => {
            const initialBalances = await getBalances(
                config.chain.source.tokens.USDC.address,
                config.chain.destination.tokens.USDC.address
            )
    ...
    describe('Cancel', () => {
        it('should cancel swap Ethereum USDC -> Monad', async () => {
            const initialBalances = await getBalances(
                config.chain.source.tokens.USDC.address,
                config.chain.destination.tokens.USDC.address
            )
