...
async function getProvider(cnf: ChainConfig): Promise<{node?: CreateServerReturnType; provider: JsonRpcProvider}> {
    console.log(`[DEBUG] Getting provider for chain ${cnf.chainId}, createFork: ${cnf.createFork}`)
    
    if (!cnf.createFork) {
        console.log(`[DEBUG] Creating direct provider for ${cnf.url}`)
        return {
    ...
    console.log(`[DEBUG] Creating fork with URL: ${cnf.url}`)
    const node = createServer({
        instance: anvil({forkUrl: cnf.url, chainId: cnf.chainId}),
        limit: 1
    })
    console.log(`[DEBUG] Starting node...`)
    await node.start()
    console.log(`[DEBUG] Node started successfully`)

    const address = node.address()
    assert(address)
    console.log(`[DEBUG] Node address: ${address.address}:${address.port}`)
