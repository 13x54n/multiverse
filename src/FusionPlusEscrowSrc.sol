...
/**
 * @title FusionPlusEscrowSrc
 * @dev Source escrow contract for 1inch Fusion+ cross-chain swaps
 * Enhanced with hashlock and timelock functionality
 */
contract FusionPlusEscrowSrc is Ownable, ReentrancyGuard {
    ...
    modifier onlyAuthorizedResolver() {
        require(authorizedResolvers[msg.sender], "FusionPlusEscrowSrc: Unauthorized resolver");
        _;
    }

    modifier escrowExists(bytes32 orderHash) {
        require(escrows[orderHash].orderHash != bytes32(0), "FusionPlusEscrowSrc: Escrow does not exist");
        _;
    }

    modifier escrowActive(bytes32 orderHash) {
        require(escrows[orderHash].isActive, "FusionPlusEscrowSrc: Escrow is not active");
        _;
    }
    ...
    modifier escrowNotExpired(bytes32 orderHash) {
        ...
        require(
            ...
            "FusionPlusEscrowSrc: Escrow expired"
        );
        _;
    }

    modifier validSecret(bytes32 secret, bytes32 hashlock) {
        require(keccak256(abi.encodePacked(secret)) == hashlock, "FusionPlusEscrowSrc: Invalid secret");
        _;
    }
    ...
    function createEscrow(
        ...
        require(amount >= minDepositAmount && amount <= maxDepositAmount, "FusionPlusEscrowSrc: Invalid amount");
        require(escrows[orderHash].orderHash == bytes32(0), "FusionPlusEscrowSrc: Escrow already exists");
        require(timelock > 0, "FusionPlusEscrowSrc: Invalid timelock");

        if (token == address(0)) {
            require(msg.value == amount, "FusionPlusEscrowSrc: Incorrect native amount");
        } else {
            require(msg.value == 0, "FusionPlusEscrowSrc: Native tokens not expected");
        ...
        escrowActive(orderHash)
    {
        ...
        require(
            msg.sender == escrow.maker || 
            block.timestamp > escrow.createdAt + escrow.timelock,
            "FusionPlusEscrowSrc: Cannot cancel yet"
        );
        ...
        validSecret(secret, escrows[orderHash].hashlock)
    {
        ...
        require(
            block.timestamp > escrow.createdAt + escrow.timelock,
            "FusionPlusEscrowSrc: Timelock not expired"
        );
